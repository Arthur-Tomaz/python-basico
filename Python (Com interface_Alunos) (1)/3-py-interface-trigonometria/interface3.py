...
# rodando o .py normalmente quanto depois de empacotados com PyInstaller.
def resource_path(relative_path):
    try:
        # PyInstaller cria um diretorio temporario e gurada o caminho em _MEIPASS
        # sys._MEIPASS é um atributo especial que o PyInstaller cria em tempo de execução quando voce empacota em script Python num execultável. É um truque para garantir que o programa encontre suas imagens
...
class CalculadoraTrigonometrica:
    ...
    def criar_imagem_topo(self):
        try:
            imagem = Image.open(resource_path("seno.png"))
            imagem = imagem.resize((380, 200), Image.Resampling.LANCZOS)
            self.criar_imagem_topo = ImageTk.PhotoImage(imagem) #guarda refêrencia
            ...
            tk.Label(
                self.janela, text="Imagem 'seno2.png' não encontrada", bg="#f0f0f0").pack(pady=20)


    # Criar onde o usuario digita o ângulo em graus (0 a 90)
    def criar_campo_entrada(self):
        frame_entrada = tk.Frame(self.janela, bg="#f0f0f0")
        frame_entrada.pack(pady=10)

        tk.Label(
            frame_entrada, text="Ângulo (0 a 90 graus):", font=("Arial", 14),bg="#f0f0f0"
            ).pack(pady=(0, 5))
        ...
        tk.Frame(frame_entrada, bg="black", height=2, width=40).pack(pady=(0, 5))
    ...
    # Labels para mostrar os resultados do seno, cosseno e tangente
    def criar_resultados(self):
        frame_resultados = tk.Frame(self.janela, bg="#f0f0f0")
        frame_resultados.pack(pady=10)

        self.resultado_seno = self._criar_resultado(frame_resultados, 0, "Seno")
        self.resultado_cosseno = self._criar_resultado(
        ...
        def _criar_linha_resultado(self, frame, linha, texto):
            tk.Label(frame, text=texto, font=("Arial", 12, "bold"), bg="#f0f0f0").grid(
            row=linha, column=0, padx=10, pady=5, sticky="e"
            )
            resultado = tk.Label(
                frame, text="", font=("Arial", 12, "bold"), fg="red", bg="#f0f0f0"
            )
            resultado.grid(row=linha, column=1, padx=10, pady=5, stickys="w")
            return resultado
        ...
        def calcular(self):
            try:
                ...
                self.resultado_tangente.config(text="Error")

        # Limpar o campo de entrada e os resultados
        def limpar(self):
